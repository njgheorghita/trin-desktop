...
// idk if we need these optional() here
  storage: z.number().min(100).optional(),
  httpPort: z.number().min(0).max(65535).optional(),
...
<template>
  <main class="min-h-screen w-full bg-background">
	...
	<Card class="p-4" v-if="trinStatus === 'running'">
	  ...
	  <CardContent>
	    <!--Progress :value="trinStats.cpu" /-->
		<p class="py-1 text-xs text-muted-foreground">Current CPU being consumed by your Trin node.</p>
	  </CardContent>
	</Card>
    <Card class="p-4">
      <CardHeader class="flex flex-row items-center justify-between pb-2">
      ...
      <CardContent>
        ...
        <p class="text-xs text-muted-foreground">Total amount of storage that will be consumed by your Trin client.</p>
      </CardContent>
    </Card>
	  <Card class="p-4">
      <CardHeader class="flex flex-row items-center justify-between pb-2">
          ...
          <div class="grid items-center gap-4">
			  ...
			  <FormField v-slot="{ field }" name="autostart" :control="form.control">
				<FormItem>
				  ...
				  <FormLabel class="text-base">Autostart</FormLabel>
				  <FormDescription>
			  ...
			  <SheetFooter class="py-6">
			    <SheetClose as-child>
			      <Button type="submit" :disabled="!form.meta.dirty">
			        Submit
			      </Button>
			    </SheetClose>
			  </SheetFooter>
